...
const { mostProducts, dailySales, profitPerProduct, totalLiquid, } = require('./order-aggregations');
...
orderSchema.statics.updateAndDeleteBottles = async function(invoice) {
  ...
  // every time you're trying to do an async function inside an array method, for loop instead
  // map with async function can go out of order
  await Promise.all(invoice.map(async orderedItem => {
  ...
  if(needsUpdate) {
    // for speed, concat into one promise.all. Use spread operator
    await Promise.all(bottlesToUpdate.map(bottle => this.updateBottle(bottle.id, bottle.newBottleAmount)));
    await Promise.all(bottlesToDelete.map(bottle => this.deleteEmptyBottle(bottle.id)));
  } else {
...
orderSchema.statics.mostProducts = async function() {
  return this.aggregate(mostProducts)
    .then(([res]) => res);
};

orderSchema.statics.dailySales = async function() {
  return this.aggregate(dailySales);
};

orderSchema.statics.profitPerProduct = async function() {
  return this.aggregate(profitPerProduct);
};

orderSchema.statics.totalLiquid = async function() {
